...
public class ExaminerQueryServiceImpl implements ExaminerQueryService {
    ...
    /**
     * Method that handles the query to get an examiner by national provider identifier
     * @param query
     * @return Optional<Examiner> - the examiner found by national provider identifier or empty if not found
     * @author Juan Astonitas
     */
    @Override
    public Optional<Examiner> handle(GetExaminerByNationalProviderIdentifier query) {
        return examinerRepository.findByNationalProviderIdentifier(
                new NationalProviderIdentifier(query.nationalProviderIdentifier())
        );
    }
}
